...
/**
 * Write cloudflared Dockerfile and tunnel startup script.
 * Uses Alpine base with cloudflared binary — the official image is scratch-based
 * (no /bin/sh), so we need a real OS for the webhook auto-update script.
 */
async function writeCloudflaredFiles(dir: string) {
  await mkdir(join(dir, "cloudflared"), { recursive: true });

  await writeFile(
    join(dir, "cloudflared", "Dockerfile"),
