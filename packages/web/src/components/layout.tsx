...
function Layout({ children, title, description, apiUrl }: LayoutProps) {
  return (
    <html lang="en" className="bg-neutral-950 text-white antialiased">
      ...
      <body className="font-sans">
        ...
        <script dangerouslySetInnerHTML={{__html: `window.__API_URL="${apiUrl || ""}";`}} />
...
export function renderPage(
  ...
  props?: { title?: string; description?: string; apiUrl?: string },
): string {
  const html = renderToString(
    <Layout title={props?.title} description={props?.description} apiUrl={props?.apiUrl}>
      {content}
    </Layout>,
  );
